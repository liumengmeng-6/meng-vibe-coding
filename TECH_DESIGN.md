...
2. **为什么**：PRD 里的 AC1（双击就能打开）和 AC14（断网也能用）把其他路线全部排除了；而且本项目就四件事（分天数、查参考价、加总、比预算），**根本用不到框架**。
   *（第二版会加一个「真实地图视图」，那是全站唯一的联网功能，对应 **AC15**；**核心流程仍然离线可用**。）*
...
| 2 | **不加载任何网络资源**，所以断网刷新照样能用（第二版那个「真实地图视图」是唯一例外，见 **AC15**，离线时置灰） | 直接满足 **AC14** |
...
## 五、文件结构与职责（三个代码文件 + 三份文档 + 一个 Skill 目录）
...
| 目录 | 放什么 | 规矩 |
|------|--------|------|
| `images/` | 城市照片 —— 首页示意地图上的圆形缩略图、清单配图 | ① 只用**可商用的免费图**（Unsplash / Pexels 等）、自拍、或官方授权图；② **每张都在本文档记下出处**；③ 是**本地文件**，浏览器直接读、不联网加载，所以**不违反 AC14** |

**为什么先把位置定死**：PRD 第二版的「示意路线地图」要用到城市缩略图，得有个固定地方放。现在写清楚目录名和三条规矩，省得以后随手新建目录、名字乱起（`AGENTS.md` 8.5 就是防这个的）。

### 项目内的 Skill（Day 12 起）

| 目录 | 放什么 | 怎么用 |
|------|--------|--------|
| `.workbuddy/skills/` | 项目自带的 Skill —— 一个文件夹 + 一份 `SKILL.md`（需要时再加 `scripts/`）。目前只有 `filter-interaction` 一个 | 要写「筛选」类交互**之前**先读它的 `SKILL.md`；写完跑 `node .workbuddy/skills/filter-interaction/scripts/check-filter.js`，这一次的结果会写进同目录的 `LAST_RUN.md` |

**它和那几份文档有什么区别**：`PRD.md` 这些是**写给人看的**；Skill 是**写给 AI 看的、而且能自己动手**——它不只列规矩，还带一个会真加载页面、真点按钮、逐项断言的检查脚本。所以「按 Skill 做了」这件事**有据可查**，不是一句口头保证。

**它不参与运行**：Skill 目录只在"开发与检查"时用，网页本身（那三个文件）完全不依赖它，**不影响 AC1 / AC14**。
...
| `README.md` | Day 7 | 运行说明：怎么打开、想改价格 / 城市数据该动哪一行 |
...
| 任何网络请求 | **断网照样能用**（满足 PRD 的 **AC14**），**双击文件就能打开**（满足 **AC1**）。第二版若加「真实地图视图」，它是全站唯一联网点，见 **AC15** |
...
| `C5 美食美景清单` | **F7**（内置清单）+ **F8**（用户自填）+ **F9**（筛选只看某类） |
...
### 约束 5：筛选类交互照 Skill 的 5 条规矩做，并用它的脚本自检（Day 12 起）

**规矩**：以后再做「筛选 / 分类切换」（把一长串内容按类型只看一部分），**先读 `.workbuddy/skills/filter-interaction/SKILL.md`**。它的 5 条，一句话版：

1. **必须有「全部」这一档** —— 只能筛进去、回不来的筛选就是死路
2. **三种状态都要有交代**：有结果（说清筛出几条）/ **没结果（必须写一句话，绝不能留一片空白）** / 恢复原样（一条不多一条不少）
3. **反馈不能只靠颜色**（色弱用户分不出深浅）—— 要有文字、形状、边界的变化；文字是最明确的一档
4. **键盘和读屏要能用**：真 `<button>`（Tab 能到、回车能触发）+ `aria-pressed` 标当前档 + `aria-live` 播报命中条数
5. **样式照约束 4 走**：颜色只用 CSS 变量、`var()` 带兜底、**两套主题对比度都达标**（正文 ≥ 4.5:1）

**它和前四条约束的区别**：约束 1–4 靠**人工自查**；这一条配了**能自己动手的检查脚本** ——

```bash
node .workbuddy/skills/filter-interaction/scripts/check-filter.js
```

它会真把页面加载起来、真去点按钮、逐项断言（目前 60 项），并把这次的结果写进同目录的 `LAST_RUN.md`。**那份文件就是「规矩确实被执行过」的证据**，比"我看过了"硬。

---

*本文件为 Day 5 产出，随项目一起提交进仓库。Day 9 追加第八节「可复用组件约束」；Day 12 追加约束 5（筛选交互照 Skill 做 + 脚本自检）。*
